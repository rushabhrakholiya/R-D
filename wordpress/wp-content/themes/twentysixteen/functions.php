...
{
	$statusArr = array("200"=>"sucess","500"=>"internal server error","404"=>"not found");
...
function elayers_func( $data ) {
	$posts = get_posts( array(
		'author' => $data['id'],
	) );

	if ( empty( $posts ) ) {		
		return array("status"=>status(404),"data"=>null);
	}
	else {
		foreach($posts as $post)
		{
			$author[] = array("title"=>$post->post_title,"content"=>$post->post_content);
		}
	}
	return array("status"=>status(200),"data"=>datafilter($author));	
}

add_action( 'rest_api_init', function () {
	register_rest_route( 'elayers/v1', '/author/(?P<id>\d+)', array(
		'methods' => 'GET',
		'callback' => 'elayers_func',			
	) );
} );
